...
@section('title', 'Bölüm Düzenle: ' . $section->title)

@section('content')
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-lg font-semibold">Bölüm Düzenle: {{ $section->title }}</h1>
            ...
            <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 bg-white dark:bg-black/20">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" required value="{{ $section->name }}" @readonly($section->is_fixed)
                            class="w-full mt-1 px-3 py-2 rounded-md border border-gray-200 dark:border-gray-800 bg-white dark:bg-black focus:ring-2 focus:ring-black dark:focus:ring-white transition-all {{ $section->is_fixed ? 'opacity-50 cursor-not-allowed' : '' }}">
                        @if ($section->is_fixed)
                            <p class="text-[10px] text-gray-500 mt-1">Sistem bölümlerinin adı değiştirilemez.</p>
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        <input name="description" rows="3" value="{{ $section->description }}"
                            class="w-full mt-1 px-3 py-2 rounded-md border border-gray-200 dark:border-gray-800 bg-white dark:bg-black focus:ring-2 focus:ring-black dark:focus:ring-white transition-all">
                    </div>

                    <div class="flex items-center gap-3 py-2">
                        <input type="checkbox" name="is_active" value="1" class="toggle" @checked($section->is_active)>
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 bg-white dark:bg-black/20">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        @php
                            $currentType = $section->data['type'] ?? 'products';
                        @endphp
                        <label
                        ...
                        <label
                            class="flex items-center gap-3 p-3 rounded-md border border-gray-100 dark:border-gray-900 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors">
                            <input type="radio" name="type" value="showcase" @checked($currentType === 'showcase')
                                class="text-black dark:text-white focus:ring-0">
                            <div>
                        ...
                        <label
                            class="flex items-center gap-3 p-3 rounded-md border border-gray-100 dark:border-gray-900 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors">
                            <input type="radio" name="type" value="blogs" @checked($currentType === 'blogs')
                                class="text-black dark:text-white focus:ring-0">
                            <div>
                        ...
                        <div class="flex flex-col">
                            ...
                            <div id="item-pool"
                                ...
                                @foreach ($products as $product)
                                    <div class="item-card group bg-white dark:bg-gray-900 p-2 rounded border border-gray-100 dark:border-gray-800 flex items-center gap-3 cursor-move hover:border-black dark:hover:border-white transition-all {{ $currentType !== 'products' && $currentType !== 'showcase' ? 'hidden' : '' }}"
                                        data-id="{{ $product->id }}" data-type="{{ $currentType }}"
                                ...
                                @foreach ($blogs as $blog)
                                    <div class="item-card group bg-white dark:bg-gray-900 p-2 rounded border border-gray-100 dark:border-gray-800 flex items-center gap-3 cursor-move hover:border-black dark:hover:border-white transition-all {{ $currentType !== 'blogs' ? 'hidden' : '' }}"
    ...
    @push('scripts')
        ...
        <script>
            ...
            $('#section-form').on('submit', function(e) {
                e.preventDefault();
                const type = $('input[name="type"]:checked').val();
                const isFixed = {{ $section->is_fixed ? 'true' : 'false' }};
                const itemIds = [];
                ...
                $.ajax({
                    url: "{{ route('admin.home-sections.update', $section->id) }}",
                    type: 'POST',
                    data: data,
