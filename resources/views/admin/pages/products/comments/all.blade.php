...
@section('title', 'Tüm Değerlendirmeler')
@section('content')
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold">Müşteri Değerlendirmeleri</h1>
    </div>
    ...
    <div id="bulk-actions" class="hidden mb-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg flex items-center gap-4">
        ...
        <div class="flex items-center gap-2">
            <button type="button" onclick="bulkAction('status_1')"
                class="px-3 py-1.5 text-xs font-semibold bg-green-600 text-white rounded-md hover:bg-green-700">Onayla</button>
            <button type="button" onclick="bulkAction('status_2')"
                class="px-3 py-1.5 text-xs font-semibold bg-red-600 text-white rounded-md hover:bg-red-700">Reddet</button>
            <button type="button" onclick="bulkAction('status_0')"
                class="px-3 py-1.5 text-xs font-semibold bg-yellow-500 text-white rounded-md hover:bg-yellow-600">Beklemeye
                Al</button>
            <button type="button" onclick="bulkAction('delete')"
                class="px-3 py-1.5 text-xs font-semibold bg-black text-white dark:bg-white dark:text-black rounded-md hover:opacity-80">Sil</button>
        </div>
    </div>
                ...
                <tr>
                    <th class="px-4 py-3 w-10">
                        <input type="checkbox" id="select-all"
                            class="rounded border-gray-300 dark:border-gray-700 text-black focus:ring-0">
                    </th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                @forelse($comments as $comment)
                    <tr>
                        <td class="px-4 py-4">
                            <input type="checkbox"
                                class="comment-checkbox rounded border-gray-300 dark:border-gray-700 text-black focus:ring-0"
                                value="{{ $comment->id }}">
                        </td>
                        <td class="px-6 py-4">
                            @if ($comment->product)
                        ...
                        <td class="px-6 py-4">
                            @if ($comment->status == 0)
                                <span
                                    class="px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800">Beklemede</span>
                            @elseif($comment->status == 1)
                                <span
                                    class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">Onaylandı</span>
                            @else
                                <span
                                    class="px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800">Reddedildi</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end items-center gap-2">
                                <a href="{{ route('admin.product-comments.show', $comment->id) }}"
                                    class="p-1 hover:text-blue-600" title="Detay"><i class="ri-eye-line"></i></a>
                                <button type="button" onclick="deleteComment({{ $comment->id }})"
                                    class="p-1 hover:text-red-600" title="Sil"><i
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-6 py-10 text-center text-gray-500">Henüz değerlendirme bulunmuyor.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @push('scripts')
        <script>
            ...
            function bulkAction(action) {
                const ids = $('.comment-checkbox:checked').map(function() {
                    return $(this).val();
                }).get();
