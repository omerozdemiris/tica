    ...
    <div class="min-h-screen flex items-center justify-center p-4 relative z-10">
        <div
            ...
            <div class="text-center mb-8">
                ...
                @endif
                <h1 class="text-2xl font-bold mb-2 text-black/70 dark:text-white/70">Yönetim Girişi</h1>
                @if ($store && $store->about)
                    <p class="text-sm text-gray-600 dark:text-gray-400">{!! strip_tags(Str::limit($store->about, 100)) !!}</p>
                @endif
            </div>
            ...
            <form method="POST" action="{{ route('admin.login.submit') }}" class="space-y-4">
                @csrf
                <div>
                    <label class="text-sm font-medium mb-1 block">E-posta veya Kullanıcı Adı</label>
                    <input type="text" name="email"
                        class="w-full px-4 py-2.5 rounded-md border border-gray-200 dark:border-gray-800 bg-white dark:bg-black focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                        placeholder="E-posta veya kullanıcı adı" required>
                </div>
                <div>
                    <label class="text-sm font-medium mb-1 block">Şifre</label>
                    <div class="relative">
                        <input type="password" name="password" id="login-password"
                            class="w-full px-4 py-2.5 pr-12 rounded-md border border-gray-200 dark:border-gray-800 bg-white dark:bg-black focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                            placeholder="Şifrenizi girin" required>
                        <button type="button" data-toggle-password
                            class="absolute inset-y-0 right-0 px-3 flex items-center text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
                ...
                <button type="submit"
                    class="w-full px-4 py-2.5 rounded-md text-sm font-medium border border-gray-200 dark:border-gray-800 bg-black text-white dark:bg-white dark:text-black hover:opacity-90 transition-opacity">
                    Giriş Yap
                </button>
            </form>
        </div>
