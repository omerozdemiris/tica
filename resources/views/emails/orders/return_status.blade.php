...
<head>
    ...
    <title>{{ config('app.name') }} - Sipariş Durumu</title>
</head>
@php
    $colors = $themeColors ?? [
        'primary' => '#2563eb',
    ...
    $statusLabels = [
        'pending' => ['label' => 'İade talebiniz alınmıştır', 'color' => '#f97316', 'bg' => '#ffedd5'],
        'processed' => ['label' => 'İade talebiniz onaylandı', 'color' => '#16a34a', 'bg' => '#dcfce7'],
        'rejected' => ['label' => 'İade talebiniz reddedildi', 'color' => '#ef4444', 'bg' => '#fee2e2'],
    ];
    $meta = $statusLabels[$status] ?? [
        'label' => 'İade durum güncellemesi',
                    ...
                    <tr>
                        <td align="center" style="padding:36px 32px;background-color:{{ $colors['primary_light'] }};">
                            ...
                            <p style="margin:0;font-size:14px;color:#475569;">
                                {{ $messageContent ?? 'Siparişinizle ilgili son durumu aşağıda bulabilirsiniz.' }}
                            </p>
                        </td>
                    </tr>
                    <tr>
                            ...
                            <div style="display:flex;flex-wrap:wrap;gap:16px;margin-bottom:24px;">
                                @if ($status === 'rejected')
                                    <div
                                        ...
                                        <p style="margin:0;font-size:12px;text-transform:uppercase;color:#94a3b8;">İade
                                            Talebi Reddedilme Sebebi
                                        </p>
                                        <p style="margin:6px 0 0;font-size:18px;font-weight:600;color:#111827;">
                                            {{ $return->reason ?? 'Sebep belirtilmedi' }}</p>
                                    </div>
                                @endif
                                <div
                                    style="flex:1;min-width:200px;padding:20px;border-radius:18px;background-color:#f8fafc;border:1px solid #e2e8f0;">
                                ...
                                <div
                                    ...
                                    <p style="margin:6px 0 0;font-size:18px;font-weight:600;color:#111827;">
                                        {{ number_format($return->order->total, 2, ',', '.') }} ₺</p>
                                </div>
                            </div>
                            {{-- @if ($return->order->method === 'wire' && isset($wireBanks) && $wireBanks->isNotEmpty())
                                <div
                                    style="background-color:#ecfdf5;border:1px solid #d1fae5;border-radius:16px;padding:20px;margin-bottom:24px;">
                                    <h2
                                        style="margin:0 0 16px 0;font-size:14px;font-weight:600;color:#064e3b;text-transform:uppercase;letter-spacing:0.025em;">
                                        Banka Bilgileri</h2>
                                    @if ($wireBanks->isNotEmpty())
                                        <div style="margin-bottom:16px;">
                                            @foreach ($wireBanks as $bank)
                                                <div
                                                    style="background-color:#ffffff;border:1px solid #d1fae5;border-radius:12px;padding:16px;margin-bottom:10px;">
                                                    <div
                                                        style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
                                                        <div>
                                                            <p
                                                                style="margin:0;font-size:10px;text-transform:uppercase;letter-spacing:0.05em;color:#059669;">
                                                                Banka</p>
                                                            <p
                                                                style="margin:2px 0 0;font-size:14px;font-weight:600;color:#111827;">
                                                                {{ $bank->bank_name }}</p>
                                                        </div>
                                                    </div>
                                                    <div style="font-size:13px;color:#374151;">
                                                        <p style="margin:0 0 4px;"><span
                                                                style="font-weight:600;">Alıcı:</span>
                                                            {{ $bank->bank_receiver }}</p>
                                                        <p
                                                            style="margin:0;font-size:11px;font-family:monospace;letter-spacing:0.05em;color:#4b5563;">
                                                            {{ $bank->bank_iban }}</p>
                                                    </div>
                                                </div>
                                            @endforeach
                                    ...
                                    @else
                                        <div
                                            style="background-color:#fefce8;border:1px solid #fef9c3;color:#713f12;padding:12px;border-radius:12px;font-size:13px;">
                                            Banka bilgileri kısa süre içinde paylaşılacaktır. Lütfen müşteri
                                            temsilcimizle
                                            iletişime geçin.
                                        </div>
                                    @endif
                                    <div style="margin-top:16px;font-size:12px;color:#065f46;">
                                        Havale/EFT dekontunu oluştururken açıklama alanına mutlaka
                                        <strong>{{ $return->order->order_number }}</strong>
                                        yazmayı unutmayın. Ödemeniz ulaştığında siparişiniz onaylanacaktır.
                                    </div>
                                </div>
                            @endif --}}
                            @if ($return->order->shipping && $return->order->shipping->shippingCompany && $status === 'completed')
                                <div
                                    style="padding:20px;border:1px solid #e0f2fe;background:#f0f9ff;border-radius:16px;margin-bottom:24px;">
                                    <h3 style="margin:0 0 10px;font-size:16px;color:#0f172a;">Kargo Bilgileri</h3>
                                    <p style="margin:4px 0;color:#0369a1;">
                                        Kargo Firması: {{ $return->order->shipping->shippingCompany->name }}
                                    </p>
                                    <p style="margin:4px 0;color:#0369a1;">
                                        Takip No: <strong>{{ $return->order->shipping->tracking_no }}</strong>
                                    </p>
                                    @if ($order->shipping->tracking_link)
                                        <p style="margin:10px 0 0;">
                                            <a href="{{ $return->order->shipping->tracking_link }}" target="_blank"
                                                style="display:inline-block;padding:10px 16px;background:{{ $colors['primary'] }};color:{{ $colors['text_on_primary'] }};text-decoration:none;border-radius:10px;font-weight:600;">
