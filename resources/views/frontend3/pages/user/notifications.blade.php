...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-4xl mx-auto">
            <h1 class="text-2xl font-bold mb-6">Bildirimlerim</h1>

            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                @forelse($notifications as $notification)
                    <div
                        class="p-4 border-b border-gray-50 flex items-start gap-4 {{ !$notification->read_at ? 'bg-blue-50/20' : '' }}">
                        <div
                            class="w-12 h-12 rounded-full {{ $theme->color ? 'bg-' . $theme->color . '/10' : 'bg-gray-600' }} flex items-center justify-center flex-shrink-0">
                            <i
                                class="ri-notification-3-line {{ $theme->color ? 'text-' . $theme->color : 'text-gray-600' }}0 text-xl"></i>
                        </div>
                        <div class="flex-1">
                            ...
                            <p class="text-gray-600 mt-1">{{ $notification->text }}</p>
                            <div class="mt-3">
