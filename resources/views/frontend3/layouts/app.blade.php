...
<head>
    ...
    <meta name="description" content="{!! strip_tags($store->meta_description ?? '') !!}" charset="utf-8">
    @if (request()->path() == '/')
...
<body class="min-h-screen bg-urgupmain text-gray-900 flex flex-col">

    @if (!$isMobile)
        @if ($header_campaign)
            @include($template . '.parts.campaings.header')
        @endif
    @endif
    @include($template . '.parts.header')
    @if ($isMobile)
        @include($template . '.parts.mobile.menu')
    @endif
    <main class="flex-1">
        @yield('content')
    </main>
    ...
    @include($template . '.parts.mobile.mobile_nav')
