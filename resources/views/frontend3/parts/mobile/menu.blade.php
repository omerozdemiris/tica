    ...
    <div class="flex-1 overflow-y-auto relative">
        <div id="mobile-menu-root" class="absolute inset-0 flex flex-col bg-white transition-transform duration-300">
            @if ($isMobile)
                @if ($header_campaign)
                    <div class="border-b border-gray-100">
                        @include('frontend2.parts.campaings.header')
                    </div>
                @endif
            @endif
            <nav class="p-4 space-y-1">
                    ...
                    <div class="border-b border-gray-100 last:border-0">
                        @if ($hasChildren)
                            <button type="button" data-open-panel="{{ $panelId }}"
                                ...
                                <span
                                    class="text-gray-700 font-semibold text-sm group-hover:text-gray-900">{{ $category->name }}</span>
                                <i class="ri-arrow-right-s-line text-gray-900 text-lg"></i>
                            </button>
                        @else
                            <a href="{{ route('categories.show', [$category->id, $category->slug]) }}"
                                class="block p-2 text-sm text-gray-800 font-semibold hover:text-gray-900">
                                {{ $category->name }}
                            </a>
                        @endif
                    </div>
                ...
                <div class="border-b- border-gray-100 last:border-0">
                    <a href="{{ route('home') }}"
                        class="block p-2 text-sm text-gray-600 font-medium hover:text-gray-900">
                        Ana Sayfa
                    </a>
                </div>
                <div class="border-b- border-gray-100 last:border-0">
